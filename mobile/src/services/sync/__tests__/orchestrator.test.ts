...
describe('SyncOrchestrator.syncNotes: 構造先行コミット (pull 中の UI 整合性)', () => {
	...
	it('toDownload が空でも cloud のフォルダ構造更新は最終 commit で反映する', async () => {
		// cloud と local の本文 hash は完全一致、noteList 構造だけ cloud 側で変わった状態。
		// この場合 download は不要だが、folderId / order は取り込む必要がある。
		const note = makeNote({ id: 'a', content: 'same' });
		cloud.setCloudNote(note);
		await notes.saveNote(note);
		await cloud.rebuildNoteListFromCloud(iso(10_000));
		cloud.noteList.folders = [
			{ id: 'f1', name: 'CloudFolder', archived: false },
		];
		cloud.noteList.notes = cloud.noteList.notes.map((n) =>
			n.id === 'a' ? { ...n, folderId: 'f1' } : n,
		);
		cloud.noteList.topLevelOrder = [{ type: 'folder', id: 'f1' }];
		...
		notes.replaceNoteList = async (list, opts) => {
			replaceCount++;
			return origReplace(list, opts);
		};

		await orch.syncNotes();

		// pre-commit はスキップ、最終 commit で cloud 構造を反映
		expect(replaceCount).toBe(1);
		const final = notes.getNoteList();
		expect(final.folders).toEqual([
			{ id: 'f1', name: 'CloudFolder', archived: false },
		]);
		expect(final.notes.find((n) => n.id === 'a')?.folderId).toBe('f1');
		expect(final.topLevelOrder).toEqual([{ type: 'folder', id: 'f1' }]);
	});
});
...
describe('SyncOrchestrator.syncNotes: ユーザー操作勝ちマージ (pull 中の dirty 検出)', () => {
	...
	it('pull 中盤でフォルダ header をドラッグしても、操作以降にダウンロードされるノートが正しいフォルダに留まる', async () => {
		// 再現: 既存ノート多数 (各フォルダ複数ノート) のクラウドに新端末でサインインし、
		// 一括 pull の途中でフォルダ header をドラッグすると、操作後にダウンロードされた
		// ノートが正しいフォルダに復元されない、というユーザー報告のリプロ。
		const folderX = { id: 'X', name: 'XF', archived: false };
		const folderY = { id: 'Y', name: 'YF', archived: false };
		// デスクトップ版の個別 note JSON は folderId を持たないため、note 本体は
		// top-level 相当、noteList metadata だけが folderId を持つ状態を再現する。
		const f1 = makeNote({ id: 'f1', folderId: '', content: 'f1' });
		const f2 = makeNote({ id: 'f2', folderId: '', content: 'f2' });
		const f3 = makeNote({ id: 'f3', folderId: '', content: 'f3' });
		const g1 = makeNote({ id: 'g1', folderId: '', content: 'g1' });
		const g2 = makeNote({ id: 'g2', folderId: '', content: 'g2' });
		const g3 = makeNote({ id: 'g3', folderId: '', content: 'g3' });
		cloud.setCloudNote(f1);
		cloud.setCloudNote(f2);
		cloud.setCloudNote(f3);
		cloud.setCloudNote(g1);
		cloud.setCloudNote(g2);
		cloud.setCloudNote(g3);
		cloud.noteList.folders = [folderX, folderY];
		cloud.noteList.topLevelOrder = [
			{ type: 'folder', id: 'X' },
			{ type: 'folder', id: 'Y' },
		];
		...
		cloud.noteList.notes = cloud.noteList.notes.map((n) => ({
			...n,
			folderId: n.id.startsWith('f') ? 'X' : 'Y',
		}));
		cloud.noteList.topLevelOrder = [
			{ type: 'folder', id: 'X' },
			{ type: 'folder', id: 'Y' },
		...
		// 中盤 (2 件 saveNoteFromSync 完了時点) でユーザー drag を割り込ませる。
		// UI は applyOptimisticNoteList → 遅延 replaceNoteList(list, {preserveExtras:true})
		// なので、ここではその呼び出しのみを reproduces する。
		let savedSoFar = 0;
		let dragApplied = false;
		const origSaveFromSync = notes.saveNoteFromSync.bind(notes);
		notes.saveNoteFromSync = async (note) => {
			await origSaveFromSync(note);
			savedSoFar++;
			if (!dragApplied && savedSoFar === 2) {
				dragApplied = true;
				// drag baseline = 現在の in-memory list (B 修正と同じ経路)
				const current = notes.getNoteList();
				const draggedList: NoteList = {
					...current,
					topLevelOrder: [
						{ type: 'folder', id: 'Y' },
						{ type: 'folder', id: 'X' },
				...
				await notes.replaceNoteList(draggedList, { preserveExtras: true });
				await state.markDirty();
			}
		};
		...
		const final = notes.getNoteList();
		// すべてのノートが正しいフォルダに居る
		expect(final.notes.find((n) => n.id === 'f1')?.folderId).toBe('X');
		expect(final.notes.find((n) => n.id === 'f2')?.folderId).toBe('X');
		expect(final.notes.find((n) => n.id === 'f3')?.folderId).toBe('X');
		expect(final.notes.find((n) => n.id === 'g1')?.folderId).toBe('Y');
		expect(final.notes.find((n) => n.id === 'g2')?.folderId).toBe('Y');
		expect(final.notes.find((n) => n.id === 'g3')?.folderId).toBe('Y');
		// 全 6 件揃っている (途中欠落していない)
		expect(final.notes.map((n) => n.id).sort()).toEqual([
			'f1',
			'f2',
			'f3',
			'g1',
	...
	it('サインイン後 bulkRepair 中にフォルダ drag が来ても、ユーザーの並び順とノート配置が両方とも保持される', async () => {
		// 新端末サインインの実フローを再現:
		// - requestBulkRepair() 済み (driveService.signIn 経由)
		// - 一括 pull 中、download 完了後に fillEmptyContentHeadersInList の await 中で
		//   ユーザーが folder を drag → merge が走る → その後の replaceNoteList(target) で
		//   drag が wiped されるかを確認する。
		const folderX = { id: 'X', name: 'XF', archived: false };
		const folderY = { id: 'Y', name: 'YF', archived: false };
		// content / contentHeader 共に空のノートを cloud に置く。
		// このとき buildMetadata の auto-generate は generateContentHeader('') = '' を返すので
		// メタデータの contentHeader も空のまま残り、bulkRepair の readNote ループが実際に走る。
		const f1 = makeNote({
			id: 'f1',
			folderId: '',
			content: '',
		...
		// signIn 経由の bulkRepair を有効化
		orch.requestBulkRepair();

		// fillEmptyContentHeadersInList が走り始めたら、その途中で drag を割り込ませる。
		// 1 件目のノート読み込み完了後にユーザー drag が起きた想定。
		let dragDone = false;
		const origReadNote = notes.readNote.bind(notes);
		let readCalls = 0;
		notes.readNote = async (id) => {
			readCalls++;
			const result = await origReadNote(id);
			if (!dragDone && readCalls === 1) {
				dragDone = true;
				const current = notes.getNoteList();
				const draggedList: NoteList = {
					...current,
