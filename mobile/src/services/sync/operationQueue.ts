...
export class OperationQueue {
  ...
  /**
   * Drive 未接続状態で executor が「Drive not connected」エラーを投げ続け、
   * 15s おきに再試行で警告ログが出るのを防ぐためのフラグ。
   * `driveService.connect()` 成功時に `resume()`、起動時 / signOut 時に `pause()`。
   */
  ...
  /** Drive 未接続中はキューを止めて executor 呼び出しを抑止する。 */
  pause(): void {
    this.paused = true;
  }

  /** 接続できたらキューを再開する。pending 項目があれば即実行される。 */
  resume(): void {
    this.paused = false;
    this.wake();
  }
  ...
  /** 端末データ削除後に、永続ファイルへ書かずキューのメモリ状態だけ空に戻す。 */
  async resetInMemory(): Promise<void> {
    await this.stop();
    await this.lock.run(async () => {
      this.items = [];
      this.nextId = 1;
  ...
  private async runLoop(): Promise<void> {
    while (!this.stopFlag) {
      if (this.paused) {
        // Drive 未接続: resume() による wake まで実行しない。
        await Promise.race([this.wakeUp, sleep(60_000)]);
        continue;
      }
