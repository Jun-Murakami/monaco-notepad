...
describe('AppBar', () => {
  ...
  it('Google Drive同期ボタンが正しく表示されること', () => {
    ...
    expect(screen.getByRole('button', { name: 'Connect to Google Drive' })).toBeInTheDocument();
  });
  ...
  it('プラットフォームに応じて正しいキーボードショートカットが表示されること', () => {
    ...
    expect(screen.getByRole('button', { name: 'New (Command + N)' })).toBeInTheDocument();

    render(<AppBar {...defaultProps} platform='win32' />);
    expect(screen.getByRole('button', { name: 'New (Ctrl + N)' })).toBeInTheDocument();
  });
});
