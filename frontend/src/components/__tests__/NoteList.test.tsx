...
describe('NoteList', () => {
  ...
  it('現在選択中のノートが強調表示されること', () => {
    ...
    const selectedNote = screen.getByText('Note 1').closest('.MuiListItemButton-root');
    expect(selectedNote).toHaveClass('Mui-selected');

    const nonSelectedNote = screen.getByText('First line Second line').closest('.MuiListItemButton-root');
